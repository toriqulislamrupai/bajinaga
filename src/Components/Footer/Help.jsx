...
const Help = ({ isModalOpen, setIsModalOpen }) => {
  ...
  const help = [
    ...
    { id: 5, title: { en: "Faq", bn: "সাধারণ প্রশ্ন" } },
    { id: 6, title: { en: "Rules and Regulation", bn: "নিয়ম ও বিধি" } },
    { id: 7, title: { en: "Responsible Gambling", bn: "দায়িত্বশীল জুয়া" } },
  ];
    ...
    <div>
      <div className="flex flex-col p-4 gap-2">
        ...
        <h3 className="text-white text-sm">
          {language === "en" ? "Help Topics" : "সাহায্য বিষয়"}
        </h3>
        <svg
      ...
      {isOpen && (
        <div className="flex flex-col text-xs p-4 gap-4">
          {help.map((item) => (
            <div key={item.id} className="rounded-lg shadow-md cursor-pointer">
              {item.link ? (
                <Link
                  to={item.link}
                  className="hover:text-opacity-100 text-white text-opacity-50 whitespace-nowrap"
